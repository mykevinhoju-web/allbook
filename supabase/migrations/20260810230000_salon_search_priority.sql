-- Super-admin search ranking boost for marketplace list order.
-- Higher search_priority appears first (still within radius / filters).

alter table public.salons
  add column if not exists search_priority integer not null default 0;

comment on column public.salons.search_priority is
  'Platform super-admin ranking boost. Higher values appear earlier in marketplace search. Default 0.';

create index if not exists salons_search_priority_idx
  on public.salons (search_priority desc)
  where marketplace_visible = true and permanently_closed = false;

drop function if exists public.search_marketplace_salons(double precision, double precision, double precision, text, text[], text, text, integer, integer);

create function public.search_marketplace_salons(
...
as $$
  ...
  order by
    coalesce(f.search_priority, 0) desc,
