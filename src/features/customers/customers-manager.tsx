      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={exportAll}
            disabled={isEmpty}
            className="inline-flex h-11 items-center gap-2 rounded-full border border-neutral-200 bg-white px-4 text-[13px] font-semibold text-neutral-800 disabled:opacity-50"
          >
      ...
      {isEmpty ? (
        <div className="rounded-[24px] border border-dashed border-neutral-300 bg-white px-6 py-16 text-center shadow-sm">
          ...
          <p className="mx-auto mt-2 max-w-md text-[14px] text-neutral-500">
            Customers appear when bookings are made, or you can add them
            manually.
          </p>
          <button
            type="button"
              ...
              editing ? (
                ...
                <CustomerProfile
                  ...
                  onAddNote={async (note) => {
                    setActionError(null);
                    try {
                      const next = await addCustomerNote(selected, note, null);
                      replaceCustomer(next);
                    } catch (err) {
                      setActionError(
                  ...
                  onCreateBooking={() =>
                    router.push("/platform/salon/bookings")
                  }
