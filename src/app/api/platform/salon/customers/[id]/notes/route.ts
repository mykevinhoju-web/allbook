...
import { addCustomerNote } from "@/features/customers/updateCustomer";
...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!body.salonId || typeof body.note !== "string") {
      return NextResponse.json(
        { error: "salonId and note are required." },
        { status: 400 },
      );
    }
    ...
    const customer = await addCustomerNote(
      supabase,
      current,
      body.note,
      body.staff ?? null,
